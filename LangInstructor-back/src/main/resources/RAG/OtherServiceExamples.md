# OtherServiceExamples.java 示例

**示例目的:**
此文件演示了 LangChain4j AI 服务的各种高级用法，包括情感分析、数字抽取、日期时间解析、POJO 结构化抽取、系统消息配置、动态消息处理等。

**关键类和方法:**
- `AiServices.create(Class, ChatModel)`: 创建 AI 服务实例，支持接口自动实现。
- `@UserMessage`、`@SystemMessage`: 定义用户和系统消息模板，支持变量插值。
- `@Description`: 为 POJO 字段添加描述，帮助 LLM 更好理解数据结构。
- `@V("name")`: 标记方法参数作为模板变量。
- `@UserName`、`@MemoryId`: 特殊注解，分别用于用户名和记忆标识。
- `responseFormat("json_schema")`、`strictJsonSchema(true)`: 强制 JSON 输出，提高结构化抽取可靠性。
- `systemMessageProvider(Function)`: 动态系统消息提供器，支持上下文相关的系统消息。

**注意事项:**
- 需要有效的 OpenAI API Key。
- 支持多种数据类型抽取（枚举、数字、日期时间、自定义对象）。
- 结构化输出推荐启用 JSON Schema 模式以提高准确性。
- 系统消息支持从资源文件加载，便于模板管理。
