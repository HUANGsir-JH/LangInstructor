# _08_AIServiceExamples.java 示例

**示例目的:**
这是 LangChain4j 教程系列的第八个示例，全面演示 AI 服务的各种功能，包括简单对话、消息模板、数据抽取、POJO 结构化输出、记忆管理等高级特性。

**关键类和方法:**
- `AiServices.create(Class, ChatModel)`: 创建 AI 服务实例，支持接口自动实现。
- `@SystemMessage`、`@UserMessage`: 定义系统和用户消息模板，支持变量插值。
- `@V("name")`: 标记方法参数作为模板变量。
- `@Description`: 为 POJO 字段添加描述，提升结构化抽取准确性。
- `@MemoryId`: 支持多用户独立记忆管理。
- `responseFormat("json_schema")`、`strictJsonSchema(true)`: 强制 JSON 输出模式。
- `MessageWindowChatMemory`: 消息窗口记忆，支持多轮对话。
- `chatMemoryProvider`: 记忆提供器，为不同用户创建独立记忆。

**注意事项:**
- 需要有效的 OpenAI API Key。
- 支持多种数据类型抽取：枚举、数字、日期时间、自定义对象、列表等。
